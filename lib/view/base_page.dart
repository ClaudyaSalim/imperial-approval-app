...
class _BasePageState extends State<BasePage> {
  ...
  void initMenu(){
    listMenu.add(MenuClass("List Request", ListRequest(), true));
    if(currUser!.role=="Approver"){
      listMenu.add(MenuClass("List Approval", ListApproval(),false));
    }

    listMenu.addAll(
      [MenuClass("List Draft", DraftRequest(), false),
      MenuClass("History", HistoryPage(), false),
      MenuClass("Logout", LoginPage(), false)]
    );
      ...
      builder: (context, snapshot) {
        if(snapshot.data!=null){
          ...
          return Scaffold(
            ...
            floatingActionButton: targetPage!.hasMainButton? FloatingActionButton(onPressed: (){Navigator.pushNamed(context, '/create-request');}, child: Icon(Icons.add_rounded),) : null,
          );
        }
        else {
